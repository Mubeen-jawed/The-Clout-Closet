...
export default function BrandPage() {
  const { brand } = useParams(); // brand slug
  const [listings, setListings] = useState([]);

  useEffect(() => {
    API.get(`/listings/brand/${brand}`).then((res) => setListings(res.data));
  }, [brand]);

  // Format slug for display (e.g., mens-clothing -> Mens Clothing)
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">{formattedBrand}</h1>
